...
文部科学省「日本食品標準成分表2023年版（八訂）」の **全2,538食品・53栄養素** を、ブラウザ上でインタラクティブに比較・可視化する静的Webアプリです。ビルド不要のバニラJavaScript（ESモジュール）＋ Chart.js で動作し、Vercel などの静的ホスティングにそのままデプロイできます。
